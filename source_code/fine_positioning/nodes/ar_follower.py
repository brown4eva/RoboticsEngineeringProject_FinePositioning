#!/usr/bin/env python
"""
    Initially created for the Pi Robot Project: http://www.pirobot.org
    Copyright (c) 2013 Patrick Goebel.

    Modified by Meldrick REIMMER, Danie SONIZARA and Selma BOUDISSA to
    solve robotic project about visual servoing in the task of fine 
    positioning.
"""
# necessary library
import rospy
...
class ARFollower():
    def __init__(self):
        ...
        self.rate = rospy.get_param("~rate", 0.6)
        r = rospy.Rate(self.rate) 
        
        # The maximum rotation speed in radians per second
        self.max_angular_speed = rospy.get_param("~max_angular_speed", 0.5) ## 2.0
        
        # The minimum rotation speed in radians per second
        self.min_angular_speed = rospy.get_param("~min_angular_speed", 0.2) ### 0.5
        
        # The maximum distance a target can be from the robot for us to track
        self.max_x = rospy.get_param("~max_x", 30.0) ### 5.0
        
        # The goal distance (in meters) to keep between the robot and the marker
        self.goal_x = rospy.get_param("~goal_x", 0.3) ### 0.5
        
        # How far away from the goal distance (in meters) before the robot reacts
        self.x_threshold = rospy.get_param("~x_threshold", 0.2) ###0.05
        
        # How far away from being centered (y displacement) on the AR marker
        # before the robot reacts (units are meters)
        self.y_threshold = rospy.get_param("~y_threshold", 0.005)
        
        # How much do we weight the goal distance (x) when making a movement
        self.x_scale = rospy.get_param("~x_scale", 0.1) #### 0.5

        # How much do we weight y-displacement when making a movement        
        self.y_scale = rospy.get_param("~y_scale", 0)
        
        # The max linear speed in meters per second
        self.max_linear_speed = rospy.get_param("~max_linear_speed", 2.0)  ########  0.3
        ...
        while not rospy.is_shutdown():
            ...
            self.cmd_vel_pub.publish(self.move_cmd) #modification remove(self.move_cmd)
    ...
    def set_cmd_vel(self, msg):
        ...
        try:
            ...
            ## Rotating in order to search for the tag
            #Rspeed = 0.0001
            #self.move_cmd.angular.z = copysign(max(self.min_angular_speed,
            #                            min(self.max_angular_speed, abs(Rspeed))), Rspeed)
        ...
        print ('target_offset_y: {}'.format(target_offset_y))
        # Get the distance of the marker from the base
        target_offset_x = marker.pose.pose.position.x
        print ('target_offset_x : {}'.format(target_offset_x))
        ...
        if abs(target_offset_x) >= 0.4:
            speed = (target_offset_x - self.goal_x) * self.x_scale * .1
            if speed < 0:
                speed *= 1
